...
export const GedModule: React.FC<GedModuleProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-cyan-950/40 p-6 rounded-2xl border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="p-2 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 rounded-xl border border-cyan-500/20">
            ...
            {allowedCategories !== 'all' && (
              <span className="text-[10px] bg-cyan-50 text-cyan-800 dark:bg-cyan-950/80 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800 px-2.5 py-0.5 rounded-full flex items-center gap-1 font-bold">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 font-black rounded-xl text-xs flex items-center gap-2 shadow-lg shadow-amber-500/20 transition-all active:scale-95 cursor-pointer"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 font-semibold rounded-xl text-xs flex items-center gap-2 transition-all active:scale-95 cursor-pointer shadow-xs"
          >
      ...
      <div className="bg-white/90 dark:bg-slate-900/90 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 space-y-3 shadow-xs backdrop-blur-md">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-700/80 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:border-cyan-500 focus:outline-none transition-colors"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-2.5 text-xs text-slate-400 hover:text-slate-700 dark:hover:text-white flex items-center justify-center p-1 rounded-md"
          ...
          <select
            ...
            className="bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-700/80 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white focus:border-cyan-500 focus:outline-none transition-colors font-medium cursor-pointer"
          >
          ...
          <div className="flex items-center bg-slate-100 dark:bg-slate-950/80 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                viewMode === 'grid' ? 'bg-cyan-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200'
              }`}
            >
              Grille
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                viewMode === 'table' ? 'bg-cyan-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200'
              }`}
            >
              Tableau
            </button>
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800/80">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              ...
              className="rounded bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-cyan-600 focus:ring-0 cursor-pointer"
            />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredDocuments.map((doc) => (
            <div
              ...
              className="card-hover-modern bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 shadow-xs cursor-pointer transition-all flex flex-col justify-between group"
            >
              <div>
                {/* Thumbnail Preview with Document Badge */}
                <div className="relative aspect-[4/3] bg-slate-100 dark:bg-slate-950 rounded-xl overflow-hidden mb-3 border border-slate-200 dark:border-slate-800">
                  <img
                  ...
                  <div className="absolute top-2 left-2">
                    <span className="px-2 py-0.5 text-[10px] font-mono font-bold bg-slate-950/80 backdrop-blur-md text-cyan-300 rounded-lg border border-cyan-800/60 shadow-xs">
                      {doc.documentNumber}
                    </span>
                  </div>
                  <div className="absolute top-2 right-2">
                    <span
                      className={`text-[9px] px-2 py-0.5 rounded-full font-bold backdrop-blur-md uppercase tracking-wider ${
                  ...
                  {/* OCR confidence chip */}
                  {doc.ocr?.confidence && (
                    <div className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded-md bg-slate-950/80 backdrop-blur-md text-[9px] font-mono font-semibold text-slate-300 border border-slate-800 flex items-center gap-1">
                ...
                {/* Details */}
                <h3 className="font-bold text-sm text-slate-900 dark:text-white line-clamp-1 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                  {doc.title}
                </h3>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 capitalize mt-0.5 font-medium">
                ...
                <div className="mt-2.5 space-y-1 text-[11px]">
                  {doc.context.projectName && (
                    <p className="text-slate-600 dark:text-slate-300 flex items-center gap-1.5 truncate font-medium">
                  ...
                  {doc.context.clientName && (
                    <p className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5 truncate font-medium">
                      <span className="text-slate-400">Client:</span>
                      <span className="truncate text-slate-700 dark:text-slate-300">{doc.context.clientName}</span>
                    </p>
                  )}
                  {doc.context.supplierName && (
                    <p className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5 truncate font-medium">
                      <span className="text-slate-400">Fourn:</span>
                      <span className="truncate text-slate-700 dark:text-slate-300">{doc.context.supplierName}</span>
                    </p>
                  )}
                </div>
              </div>
              ...
              {/* Bottom Footer Info */}
              <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400">
        ...
        <div className="bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl overflow-hidden shadow-xs">
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead className="bg-slate-50/90 dark:bg-slate-950/90 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 font-bold uppercase text-[11px]">
                <tr>
                  <th className="py-3.5 px-4">N° Document</th>
                  <th className="py-3.5 px-4">Titre &amp; Catégorie</th>
                  <th className="py-3.5 px-4">Projet Chantier</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filteredDocuments.map((doc) => (
                  <tr
                    key={doc.id}
                    onClick={() => onSelectDocument(doc)}
                    className="hover:bg-cyan-500/5 dark:hover:bg-cyan-400/5 cursor-pointer transition-colors"
                  >
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 capitalize">{doc.category.replace('_', ' ')}</p>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 transition-colors"
                        title="Visualiser"
                      >
      ...
      {filteredDocuments.length === 0 && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center space-y-3 shadow-xs">
          ...
          <h3 className="font-bold text-base text-slate-900 dark:text-white">Aucun document ne correspond à vos critères</h3>
          ...
          <div className="pt-2">
            <button
              onClick={onOpenScanner}
              className="px-4 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 font-bold rounded-xl text-xs shadow-md transition-all active:scale-95 cursor-pointer"
            >
